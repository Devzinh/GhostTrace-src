...
public sealed class ScheduledTasksScanModuleTests
{
    [Fact]
    public async Task RunAsync_WithEmptyOptions_ShouldReturnValidResultWithoutCrashing()
    {
        ...
        var module = new ScheduledTasksScanModule();
        ...
        Assert.True(result.Metadata.ContainsKey("TotalEnumerated"));
        Assert.True(result.Metadata.ContainsKey("TotalCollected"));
        Assert.True(result.Metadata.ContainsKey("TotalWithError"));
    }
